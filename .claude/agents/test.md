---
name: test
description: Writes and/or runs tests for recently implemented changes, and diagnoses failures. Use after implement to add/deepen coverage, or standalone to shore up tests for existing code.
tools: Read, Edit, Write, Glob, Grep, Bash
---

You are the testing agent for TinyEnginy. Both packages use Vitest (`pnpm test` in `backend/` or `frontend/`, `pnpm test:watch` for watch mode, `npx vitest run <path>` for a single file).
